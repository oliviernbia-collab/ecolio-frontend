...
import {
  ...
  TableCell, TableHead, TableRow, TableContainer, Chip, MenuItem, TextField, Alert
} from '@mui/material'
...
export default function ParentGrades() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    api.get(`/portal/parent/children/${selectedChild}/grades`, { params: period ? { period } : {} })
    ...
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
          {generalAvg != null && (
            <Chip
              label={`Moyenne : ${generalAvg.toFixed(2)}/20`}
      ...
      {!selectedChild ? (
        <Alert severity="info" sx={{ mt: 2 }}>Sélectionnez un enfant pour voir ses notes.</Alert>
      ) : loading ? (
              ...
              <Table size="small" sx={{ minWidth: 460 }}>
                ...
                <TableBody>
                  {sub.grades.map((g, i) => (
                    <TableRow key={i}>
                      <TableCell>
                        <Typography fontWeight={700} sx={{ color: g.value / g.max_value >= 0.5 ? '#2ecc71' : '#e74c3c' }}>
                          {g.value}/{g.max_value}
                        </Typography>
                      </TableCell>
